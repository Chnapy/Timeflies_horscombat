...
public class AStarPathFinder<T> implements PathFinder<T> {
	 ...
	 */
	private TileBasedMap<T> map = null;
	/**
	 ...
	 *
	 * @param heuristic         The heuristic used to determine the search order
	 *                          of the map
	 * @param map               The map to be searched
	 * @param maxSearchDistance The maximum depth we'll search before giving up
	 * @param allowDiagMovement True if the search should try diaganol movement
	...
	public AStarPathFinder(TileBasedMap map, int maxSearchDistance,
		...
		this(maxSearchDistance, allowDiagMovement, heuristic);
		setMap(map);
	}
	...
	public final void setMap(TileBasedMap map) {
		this.map = map;
...
//		ArrayList<T> output = new ArrayList();
		...
		while (target != nodes[sx][sy]) {
			output.add(0, map.getPathPoint(target.x, target.y));
			target = target.parent;
		}
		output.add(0, map.getPathPoint(sx, sy));
